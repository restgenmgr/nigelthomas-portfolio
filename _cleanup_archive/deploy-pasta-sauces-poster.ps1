...
<div id="nt-poster">
  <div class="poster-head">
    ...
    <p>Composition, allergens & traditional pairing — at a glance</p>
  </div>
  <div class="poster-grid">
    <div class="p-row">
      ...
      <div>
        ...
        <div class="p-desc">Slow-braised minced meat & tomato, red wine, soffritto. Holds well on a steam table.</div>
        <div class="p-tags">
    ...
    <div class="p-row">
      ...
      <div>
        ...
        <div class="p-desc">Raw basil, pine nuts, Parmesan, olive oil, garlic. Never cooked — finished à la minute.</div>
        <div class="p-tags">
    ...
    <div class="p-row">
      ...
      <div>
        ...
        <div class="p-desc">Egg & Pecorino/Parmesan emulsion off direct heat, guanciale or pancetta. No cream in the authentic version.</div>
        <div class="p-tags">
...
if (-not $content.Contains($oldFigure)) {
    Write-Host "NO MATCH on the <figure> block — nothing was changed. Paste the current block back to Claude for a re-check." -ForegroundColor Yellow
    exit 1
}
...
# ---------- 2. Inject poster CSS just before </style> (first occurrence, the main stylesheet) ----------
...
# Only touch the FIRST </style> tag (the main site stylesheet, not the later .nt-infomatics-* style block)
$idx = $content.IndexOf('</style>')
if ($idx -lt 0) {
    Write-Host "No </style> tag found — CSS not injected. Add it manually." -ForegroundColor Yellow
} else {
...
Write-Host "`nDone. File size: $before bytes -> $after bytes." -ForegroundColor Cyan
Write-Host "Now diff/review before committing: git diff types-of-pasta-sauces.html" -ForegroundColor Cyan
