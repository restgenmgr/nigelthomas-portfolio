...
#
# v3 change: every special character this script needs to match
# (Â, Ã, â, the leaked BOM sequence) is built from explicit Unicode
# code points instead of being typed literally into this file. This
# makes the script's own behavior independent of how PowerShell
# happens to read THIS FILE's encoding on your machine - it only
# depends on how it reads the .html files it processes, which uses
# .NET's reliable UTF-8 auto-detection.
#
...
#
# Usage:
#   .\fix-encoding-sitewide-v3.ps1                 # scan only, writes report
#   .\fix-encoding-sitewide-v3.ps1 -Apply           # actually fixes files
#
...
Write-Host "=== Encoding Corruption Scan (v3 - code-point safe) ===" -ForegroundColor Cyan
...
# Build the indicator characters from explicit code points (not literals):
#   0x00C2 = Â    0x00C3 = Ã    0x00E2 = â
$charAcirc  = [char]0x00C2
$charAtilde = [char]0x00C3
$characirc  = [char]0x00E2
$indicatorPattern = [regex]("[" + $charAcirc + $charAtilde + $characirc + "]")

# The leaked-BOM sequence, built the same safe way: 0x00EF 0x00BB 0x00BF = "ï»¿"
$bomLeak = -join @([char]0x00EF, [char]0x00BB, [char]0x00BF)
$feffChar = [char]0xFEFF

# Patterns that indicate leftover script text embedded in the HTML content
# (not an encoding problem - a content problem). These are plain ASCII,
# so no code-point workaround needed here.
$scriptLeakPatterns = @(
    "Out-File\s+-FilePath",
    "Write-Host\s+[`"']",
    "^\s*#\s*Create\s+",
...
$report.Add("Encoding Corruption Scan Report (v3) - $timestamp")
...
foreach ($file in $files) {
    ...
    # as literal text at the start of the file. Built from code points
    # above, so this comparison is reliable regardless of script encoding.
    if ($current.Length -ge 3 -and $current.Substring(0,3) -eq $bomLeak) {
        $current = $current.Substring(3)
    }
    if ($current.Length -ge 1 -and $current[0] -eq $feffChar) {
...
if (-not $Apply -and $filesWithIssues -gt 0) {
    ...
    Write-Host "  .\fix-encoding-sitewide-v3.ps1 -Apply" -ForegroundColor Yellow
}
