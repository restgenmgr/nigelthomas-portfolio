...
.PARAMETER ExcludeFolders
    Folder name fragments to exclude from the scan (matched anywhere in the file's full path).
    Defaults to common backup folders used in this project.
...
if ($ExcludeFolders -and $ExcludeFolders.Count -gt 0) {
    ...
    $excludedCount = $allHtmlFiles.Count - $htmlFiles.Count
}

if ($htmlFiles.Count -eq 0) {
    Write-Warning "No .html files found under $Path (after exclusions)"
    return
}

if ($excludedCount -gt 0) {
    Write-Host "Excluded $excludedCount file(s) under: $($ExcludeFolders -join ', ')" -ForegroundColor DarkGray
}
